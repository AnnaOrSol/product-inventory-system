...
interface Props {
    availableProducts: GenericProduct[];
    onItemsAdded: () => void;
...
export const RequirementSelectionGrid = forwardRef<GridRef, Props>(
    ({ availableProducts, onItemsAdded, onSelectionChange }, ref) => {
        ...
        const handleBulkSave = async (itemsToSave = selectedItems) => {
            ...
            try {
                const payload = entries.map(([id, data]) => ({
                    genericProductId: Number(id),
                    genericProductName: data.name,
        ...
        const toggleProduct = (product: GenericProduct) => {
        ...
        const filteredProducts = useMemo(() => {
            return availableProducts
                .filter((p) => {
                    ...
                    const matchesCategory = p.categoryDisplayName
                        .toLowerCase()
                        .includes(searchQuery.toLowerCase());
                    const notSelected = !selectedItems[p.id];
            ...
            <div className="space-y-4">
                <div className="sticky top-0 z-20 bg-white pb-2 space-y-2">
                    <div className="relative">
                        ...
                        <Input
                            placeholder="Search generic product catalog..."
                    ...
                    <div className="flex flex-wrap gap-2 min-h-[32px]">
                        {Object.entries(selectedItems).map(([id, data]) => (
                            <div
                                key={id}
                                className="flex items-center gap-2 bg-indigo-50 text-indigo-600 text-xs font-bold px-3 py-1.5 rounded-xl border border-indigo-100 animate-in zoom-in duration-200"
                            >
                                <span>
                                    {data.name} ({data.qty})
                                </span>
                                <button onClick={() => removeSelectedItem(Number(id))}>
